<template>
	<view>
		<view class="list padding-content flex-con" v-if="!nodata">
			<view class="list-item font-thin">用户名</view>
			<view class="list-item font-thin">消费额</view>
			<view class="list-item font-thin">佣金</view>
			<view class="list-item font-thin">日期</view>
		</view>
		<view class="list padding-content flex-con" v-for="(item, index) in list" :key="index">
			<view class="list-item font-weg line-clamp-1">{{item.nickname}}</view>
			<view class="list-item font-weg">{{item.order_total}}</view>
	...
	export default {
		...
		methods: {
			getList() {
				var that = this
				this.nodata = false
				this.$api.get('/api/finance/balanceLog', {
